...
var myKey: string = "0d93d33add804b55bd80ab113a76ec7c"; //FaceAPI key1
var myKey2: string = "fbc6d14b131f44998dc56c4237dba6ca"; //FaceAPI key 2
...
var imgSelector1 : HTMLInputElement = <HTMLInputElement> $("#my-file-selector1")[0];
var imgSelector2 : HTMLInputElement = <HTMLInputElement> $("#my-file-selector2")[0];
var refreshbtn = $("#refresh-btn")[0];
var results =$("#results")[0];
var analysebtn = $("#analyse")[0];

//set up global vars
var file1Selected:boolean = false;
var file2Selected:boolean = false;
var pic1IsPhoto:boolean = false;
var pic2IsPhoto:boolean = false;
...
imgSelector1.addEventListener("change", function () { //check file1 format
    file1Selected = true;
    file2Selected = false;
...
imgSelector2.addEventListener("change", function () { //check file2 format
    file2Selected = true;
...
/**sendEmotionRequest(file, function (emotionScores) { //here we send the API request and get the response
    ...
    sendFaceDetect(photo2,function(faceID){ //MAKE first one work then uncomment this
    });
...
function checkImage(callback) : void {

    if(file1Selected==true){ //Checking 1st file OR
    var file = imgSelector1.files[0];
    }else{ // are we checking 2nd picture
...
function sendFaceDetect(file, callback) : void {
...
class Face {
    face_id:string;
    face_url:string; //you or other/family/stranger
